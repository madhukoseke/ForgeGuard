...
### Phase B — Fail-closed production — done

**Goal:** Misconfiguration cannot silently become a memory demo in production.

- `FORGEGUARD_STRICT_CONFIG` defaults **on** when `isProduction()` (override with `=0`)
- Explicit `postgres` / `insforge` store or backend without credentials throws `ForgeGuardConfigError` (no silent memory fallback)
- Migration notes in [DEPLOYMENT.md](./DEPLOYMENT.md)

**Done when:** Production with `FORGEGUARD_STORE=postgres` and no `DATABASE_URL` fails readiness / refuses to serve mutations.

---

### Phase C — Operator identity (trustworthy approvals) — C1 done

**Goal:** Approvals are attributable; not one shared secret pretending to be many people.

**MVP (C1) — shipped:**
- Named operators via `FORGEGUARD_OPERATORS` JSON and/or `FORGEGUARD_OPERATOR_TOKEN` + `FORGEGUARD_OPERATOR_ID` / `_NAME`
- `reviewed_by` set server-side from authenticated identity (client `reviewed_by` ignored)
- Timing-safe token match; local-dev identity when no tokens in non-production

**Later (C2):**
- Roles: `viewer` / `reviewer` / `admin` (demo reset = admin)
- Session cookie instead of long-lived localStorage
- Optional OIDC/SSO; hashed-at-rest secrets

**Done when (C1):** Audit rows show a real operator id that the client cannot forge.

---

### Phase D — Operator dashboard (beyond demo) — done

**Goal:** Real ops can find, act on, and export work — not only press `D`.

**Shipped:**
- Free-text + date + agent filters on the trail
- Bulk approve low-risk / reject high-risk pending (with confirm)
- CSV/JSON export of the current view
- `FORGEGUARD_PENDING_WEBHOOK_URL` on new pending ops
- Faster actions poll (2s) while `summary.pending > 0`

**Later polish:** SSE/WebSocket live trail; Slack-specific formatting.

**Done when:** An operator can clear a backlog without one-click-per-row only, and get notified when something new needs review.

---

### Phase E — Stronger guard core — done

**Goal:** Close the documented regex bypass gap enough for serious production use.

**Shipped:**
- AST path via `pgsql-ast-parser` (`lib/sql-ast.ts`) for prefilter + policy table/statement detection; regex fallback when parse fails
- `approval_threshold` in `forgeguard.config.json` (default `medium`)
- Advisory agent/session write-burst signals (`anomaly_write_burst_*`)
- Optional `blast_radius_probe` / `FORGEGUARD_BLAST_RADIUS=1` → `count(*)` estimate on execute

**Files:** [`lib/sql-ast.ts`](../lib/sql-ast.ts), [`lib/prefilter.ts`](../lib/prefilter.ts), [`lib/policy.ts`](../lib/policy.ts), [`docs/THREAT_MODEL.md`](./THREAT_MODEL.md)
...
**Next implementation step:** Phase F — library API + schema hygiene.
